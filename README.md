...
| `csrf_expiration` | `0` | `0`（既定のまま） | 後述 |

自動化を使わず `Form::csrf()` と `Security::check_token()` を明示的に書いているのは、**検証失敗時のUXを画面ごとに制御するため**です。コード量は増えますが、利用者が入力内容を失わずに再送信できます。

#### `csrf_expiration` とトークンの寿命

`csrf_expiration` は、トークンを保存するcookieに渡す有効期限（秒）です。`0` を指定すると `setcookie()` に期限を設定しないため、ブラウザを閉じると消えるセッションcookieになります。

```php
// core/classes/security.php … トークンはcookieに保存される
\Cookie::set(static::$csrf_token_key, static::$csrf_token, $expiration);

// core/classes/cookie.php … 0 は期限を設定せずそのまま setcookie に渡る
$expiration = $expiration > 0 ? $expiration + time() : 0;
return setcookie($name, $value, $expiration, ...);
```

トークンの値そのものは、`Security::fetch_token()` がリクエスト内の初回呼び出しで必ず `set_token(true)` を通るため、**画面を描画するたびに再生成されます**。トークンを再利用する `set_token(false)` は、コアとパッケージのどこからも呼ばれていません。

#### 未解決の課題: 複数タブでの送信が失敗する

上記の「描画のたびに再生成」により、**最後に描画した画面のトークンだけが有効**な状態になっています。そのため次の操作で送信が失敗します。

1. タブAで登録フォームを開く
2. タブBで一覧など別の画面を開く（この時点でトークンが再生成される）
3. タブAから送信すると「セッションの有効期限が切れました」になる

実機で再現を確認済みです。ただし検証失敗時に入力内容を保持したまま画面を再表示する作りにしているため、**もう一度送信すれば通ります**（入力のやり直しは発生しません）。

対応案として、リクエストの早い段階で `Security::set_token(false)` を呼びセッション単位のトークンにする（POST成功時のみ `csrf_rotate` で再生成する）方法を検討しています。他のフレームワークもセッション単位のトークンが一般的なためです。現状の「描画ごとに再生成」を維持すべきかを含め、方針を相談中のため未対応としています。
